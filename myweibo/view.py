...
def transfer(request):
    ...
    #下面生成评论列表的数据
    commentList = c.getallcomment(wid)
...
def MySpace(request):
    ...
    if fans['status'] != "1":
        ...
        for i in range(len(fans)):
            ...
            info['lid'] = fid
            info['rid'] = uid
            fan['isFriend'] = int(a.is_friend(info)['status'])
            fanList.append(fan)
    d["fanList"] = fanList
...
def OtherSpace(request):
    ...
    if friends['status'] != "1":
        ...
        for i in range(len(friends)):
            ...
            info['lid'] = fid
            info['rid'] = mid
            friend['isFriend'] = int(a.is_friend(info)['status'])
            friendList.append(friend)
    d["friendList"] = friendList
    ...
    uname = u.getUserName(uid)
    if uname['status'] == "1":
        master['username'] = uname['name']
    uinfo = u.getUserInfo(uid)
    if uinfo['status'] == "1":
        uinfo = uinfo['content']
    ...
    info['lid'] = uid
    info['rid'] = mid
    master['isFriend'] = int(a.is_friend(info)['status'])
...
def Recommend(request):
    ...
    weibolist = sorted(weibolist, key=lambda d: datetime.datetime.strptime(d["createTime"], "%Y-%m-%d_%H:%M:%S"), reverse=True)
...
def LoginCheck(request):
    ...
    if flag == 1:
        ...
        request.session["username"] = u.getUserName(str(uid))['name']
